-- Skateboard.bio Database Schema
-- This schema extends the existing Domain Steward database with skateboarder-specific tables
...
-- Add skater_id to domains table if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'domains' AND column_name = 'skater_id'
  ) THEN
    ALTER TABLE domains ADD COLUMN skater_id VARCHAR REFERENCES skaters(id) ON DELETE SET NULL;
  END IF;
END $$;

-- Add host column to domains if it doesn't exist (for multi-domain lookup)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'domains' AND column_name = 'host'
  ) THEN
    ALTER TABLE domains ADD COLUMN host TEXT;
    -- Populate host from name column
    UPDATE domains SET host = name WHERE host IS NULL;
...
-- Sample data insert for testing (Tony Hawk example)
-- Uncomment to add sample data:
/*
INSERT INTO skaters (slug, full_name, nickname, bio, birth_date, birthplace, hometown, stance, turned_pro_year, sponsors, profile_image_url)
VALUES (
  'tonyhawk',
  'Tony Hawk',
  'The Birdman',
  'Professional skateboarder, entrepreneur, and the most recognized action sports figure in the world.',
  '1968-05-12',
  'San Diego, California',
  'San Diego, California',
  'goofy',
  1982,
  ARRAY['Birdhouse', 'Vans', 'Independent', 'Bones'],
  'https://cdn.skateboard.bio/tonyhawk/profile.webp'
